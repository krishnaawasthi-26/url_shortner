# URL Shortner (Spring Boot + MongoDB)
...
- Store short code -> original URL mapping in MongoDB Atlas (`url_mappings` collection).
...
This project reads the URI from `MONGODB_URI`.

Default fallback URI in `application.properties`:

`mongodb+srv://url_shortner:xxx@urlshortner.s5fvbcu.mongodb.net/url_shortner?retryWrites=true&w=majority&appName=urlshortner`

## Run locally
```bash
export MONGODB_URI='mongodb+srv://url_shortner:xxx@urlshortner.s5fvbcu.mongodb.net/url_shortner?retryWrites=true&w=majority&appName=urlshortner'
mvn spring-boot:run
```
